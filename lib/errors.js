...
const mapLenzError = (z, err) => {
  ...
  // Under OAuth a 401 almost always means "the one-hour access token expired".
  // RefreshAuthError tells Zapier to run refreshAccessToken and retry. Zapier's
  // own middleware raises it only for z.request, and every call here goes
  // through the lenz-io SDK, so without this line a token would expire and
  // never refresh. Unconditional: a 2.x version is OAuth-only. A grant that
  // really is dead (revoked) fails the REFRESH instead, and Zapier marks the
  // connection stale and asks the user to reconnect.
  if (err instanceof LenzAuthError && err.statusCode === 401) {
    throw new z.errors.RefreshAuthError(withRequestId(err, 'Your Lenz sign-in has expired.'));
  }
  ...
  // reach Lenz could run the work twice. /verify carries a per-task callback
  // URL that separates runs; /assess debits before the panel runs, so
  // creates/assess.js sends an Idempotency-Key derived from the Zap, the
  // input and the hour (#19) — the replay then gets the stored answer rather
  // than a second charge. Capping the SDK's own retries (client.js) closes
  // the other half of the same problem. The remaining window is a replay
  // that straddles the top of the hour; see replayKey there.
